...
import cv2
import sys
from utils import set_camera_intrinsics,set_camera_pose
import numpy as np
...
class StereoInteractorStyle(vtkInteractorStyleTrackballActor):
    ...
    def setLeftProjection(self):
        print(f'Left fx={self.l_fx}, fy={self.l_fy}, cx={self.l_cx}, cy={self.l_cy}')
        print(f'Right Tx = {self.t_x}, fx={self.r_fx}, fy={self.r_fy}, cx={self.r_cx}, cy={self.r_cy}')
    ...
    def display_colour_diff(self):
        ...
        left_zbuffer = left_depth.GetOutput()
        left_zbuffer_range = left_zbuffer.GetScalarRange()
        print("Left ZBuffer scalar range = {}".format(left_zbuffer_range))

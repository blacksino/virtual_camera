...
from utils import *
from FPS import FarthestPointSampler
...
if __name__ == '__main__':
    ...
    K = np.array([
        [9.964e+02, 0.0, 3.750e+02],
        [0.0, 9.964e+02, 2.402e+02],
        [0.0, 0.0, 1.0, ]]
    )

    r_mat = np.linalg.inv(extrinsics[:3, :3])
    rvec = cv2.Rodrigues(r_mat)[0]
    tvec = -r_mat @ extrinsics[:3, 3]
    projected_points = cv2.projectPoints(scene_points, rvec, tvec, K, None)[0].reshape(-1, 2)
    # flip projected points
    projected_points[:, 0] = mask.shape[1] - projected_points[:, 0]
    ...
    ax[1].scatter(mask_points[:, 0], mask_points[:, 1], s=2)
    # set ax[1] to be the same scale as ax[0]
    ax[1].set_xlim(ax[0].get_xlim())
    ax[1].set_ylim(ax[0].get_ylim())
    ax[1].set_aspect('equal')
    ax[2].scatter(projected_points[:, 0], projected_points[:, 1], s=2)
    ...
    sampler = FarthestPointSampler(mask_points, 1.2*projected_points.shape[0])
    mask_points = sampler.sample()

    # normalize mask points and projected points
    baricenter = np.mean(mask_points, axis=0)
    scale = np.max(np.linalg.norm(mask_points - baricenter, axis=1))

    mask_points = mask_points - np.mean(mask_points, axis=0)
    mask_points = mask_points / np.max(np.linalg.norm(mask_points, axis=1))

    projected_points = projected_points - np.mean(projected_points, axis=0)
    projected_points = projected_points / np.max(np.linalg.norm(projected_points, axis=1))

    projected_shape = OrientedShape(projected_points)
    correspondence_matrix, minimum_energy = projected_shape.preprocess_for_dp_matching(mask_points)
    # do farthest point sampling

    fig = plt.figure()
    X = projected_points
    Y = mask_points
    plt.scatter(X[:, 0], X[:, 1], c='r', s=10, label='projected points')
    plt.scatter(Y[:, 0], Y[:, 1], c='b', s=10, label='label points')
    ...
    for i in range(correspondence_matrix.shape[0]):
        for j in range(correspondence_matrix.shape[1]):
            if correspondence_matrix[i, j] == 1:
                plt.plot([Y[j, 0], X[i, 0]], [Y[j, 1], X[i, 1]], 'g', linewidth=0.5)
    # keep x y scale same
    plt.xlim(-1, 1)
    plt.ylim(-1, 1)
    ...
    # 2d points corresponding to the projected points in mask
    corresponding_points = np.zeros((projected_points.shape[0], 2))
    for i in range(correspondence_matrix.shape[0]):
        for j in range(correspondence_matrix.shape[1]):
            if correspondence_matrix[i, j] == 1:
                corresponding_points[i, :] = mask_points[j, :]
    # rescale all points to original scale
    X = X * scale + baricenter
    Y = Y * scale + baricenter
    corresponding_points = corresponding_points * scale + baricenter

    transform = cv2.solvePnPRansac(scene_points, corresponding_points, K, distCoeffs=None, reprojectionError=1)
    new_points = cv2.projectPoints(scene_points, transform[1], transform[2], K, distCoeffs=None)[0].reshape(-1, 2)
    # flip new points
    # new_points[:, 0] = img.shape[0] - new_points[:, 0]
    # new_points[:, 1] = img.shape[1] - new_points[:, 1]


    plt.scatter(X[:, 0], X[:, 1], c='r', s=10,label='projected points')
    plt.scatter(Y[:, 0], Y[:, 1], c='b', s=10,label='label points')
    plt.scatter(new_points[:, 0], new_points[:, 1], c='g', s=10,label='PnP')
    ...
    # convert transform to extrinsics
    r_mat = cv2.Rodrigues(transform[1])[0]
    tvec = transform[2]
    extrinsics = np.eye(4)
    extrinsics[:3, :3] = r_mat

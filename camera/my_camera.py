...
class Camera_VTK:
    ...
    def key_press_call_back(self, obj, en, ):
        ...
        if key == 'space':
            ...
            # self.silhouette.GetEnableFeatureAngle()
            # self.sil_filter = vtk.vtkOutlineFilter()
            # self.sil_filter.SetInputConnection(self.iren.GetInteractorStyle().mesh)
            # self.sil_filter.Update()

            # vtk.vtkOutlineGlowPass()
            ...
            # self.renderer.RemoveActor(self.stlActor)
            self.renderer.Render()
        return
